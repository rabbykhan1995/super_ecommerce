...
const SaleQuotationInvoice = () => {
  const { id } = useParams();
  const [data, setData] = useState<any>(null);
  const [approveModal, setApproveModal] = useState<boolean>(false);
  const printRef = useRef<HTMLDivElement>(null);
  const fetchSaleDetails = async () => {
    const res = await api(`/quotation/sale-quotation-invoice/${id}`)
  ...
  return (
    <div className='px-2' ref={printRef}>
      ...
      <div id='no-print' className='flex justify-between'>
        <button onClick={handlePrint} className='global_button my-5 sm:w-fit w-full'>Print Invoice</button>

        {data?.status === "pending" && <button
         onClick={() => {
         setApproveModal(true)
        }}
         className='global_button my-5 sm:w-fit w-full'>Approve</button>}
...
export default SaleQuotationInvoice