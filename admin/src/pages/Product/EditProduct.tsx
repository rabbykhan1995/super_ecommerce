...
export default function EditProduct() {
  ...
  const updateProduct = async () => {
    ...
    for (const v of variants) {
      if ((v as any).id) {
        const old = oldVariantMap.get((v as any).id);
        if (!old) continue;

        const fields: Record<string, any> = { id: (v as any).id };
        let changed = false;
        ...
        changedVariants.push({
          ...
          images: v.images,
          imageFileIds: (v as any).imageFileIds,
        });
      }
    }
          ...
          keyExtractor={(_, i) => i} columns={[
            ...
            },
            {
              header: "Action", accessor: (row, i) =>
                variants.length > 1 && !(row as any).id && <button onClick={() => handleRemoveVariant(i as number)}>Remove</button>
              , className: "text-center"
            },
          ]} />
