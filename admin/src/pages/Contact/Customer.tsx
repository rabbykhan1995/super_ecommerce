...
import type { Contact, PaginatedResult, SearchParams } from "../../types/type";
...
const typeOptions: string[] = ["customer", "both"];
export default function Customer() {
  ...
  const fetchContacts = async () => {
    const res = await api("/contact/list", {
      params: { search: params.search, limit: params.limit, page: params.page, type: "customer" },
    });
  ...
  const handleEdit = (contact: Contact) => {
    ...
    setForm({
      ...
      type: contact.type === "supplier" ? "customer" : contact.type,
    });
  ...
  return (
    <div className="space-y-4">
          ...
          },
          {
            ...
            accessor: (row) => (
              <div className="flex gap-2 justify-end">
                ...
                <Link to={`/contact/customer-ledger/${row._id}`} className="global_button bg-green-400">
                  <History size={18} />
                </Link>
              </div>
            ),
      ...
      />
      {!!openTransactionModal && <TransactionModal contact={selectedContact} open={openTransactionModal} onClose={() => setOpenTransactionModal(false)} type={"customer"} />}
    </div>
  );
}