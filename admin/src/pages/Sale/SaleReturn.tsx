...
import type { Account, AccountOption, SaleReturnProduct } from "../../types/type";
...
export default function SaleReturn() {
  useSignals();
  const selectedProducts = useSignal<SaleReturnProduct[]>([]);
  const { id } = useParams();
  const navigate = useNavigate();

  const [sale, setPurchase] = useState<any | null>(null);
  const [saleReturnDate, setSaleReturnDate] = useState<Date>(new Date());
  ...
  const fetchSale = async () => {
    const [purchaseRes, batchRes] = await Promise.all([
      api(`/sale/saleByID/${id}`),
      api(`/sale-return/product-by-id/${id}`)
    ]);
  ...
  useEffect(() => {
    Promise.all([fetchSale(), fetchAccounts()]);
  }, [id]);
  ...
  const totalReturnAmount = selectedOnly.reduce(
    (acc, p) => acc + p.qty * p.salePrice, 0
  );
  const paid = selectedAccounts.reduce((acc, a) => acc + (a.amount || 0), 0);
  const supplierBalance = sale?.supplier?.balance ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    for (const p of selectedOnly) {
      ...
      if ((p.qty + p.saleReturnedQty) > p.soldQty) return toast.error(`Max returnable qty is ${p.remainingQty} for ${p.name}`);
    }
  ...
  if (!sale) return <div className="p-6 text-gray-400">Loading...</div>;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="relative w-full">
          ...
          <div className="relative">
            ...
            <DatePicker
              selected={saleReturnDate}
              onChange={(date: Date | null) => setSaleReturnDate(date as Date)}
      ...
      <Table
        ...
        columns={[
       ...
       
          {
            ...
            headerClassName: "text-center",
            accessor: (row) => <span>৳{row.salePrice}</span>,
          },
          {
            header: "Sold",
            className: "text-center",
            headerClassName: "text-center",
            accessor: (row) => <span>{row.soldQty}</span>,
          },
          {
            header: "Returnable",
            className: "text-center",
            headerClassName: "text-center",
            accessor: (row) => (
              <span className="text-green-600 font-medium">{row.soldQty- row.saleReturnedQty}</span>
            ),
          },
          {
            header: "Return Qty",
          ...
          },
          {
            ...
            accessor: (row) => {
              if (!row.selected) return <span className="text-gray-300">—</span>;
              return <span>৳{(row.qty * row.salePrice).toFixed(2)}</span>;
            },
          },
        ]}
      />
