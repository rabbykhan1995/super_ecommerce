# Task 33 — Product Detail Screen

> **Phase**: 9 — Product Screens
> **Say**: "generate task 33" or "generate task 33"

## Objective

Create the product detail screen with image gallery, variants, and add to cart.

## Reference

`/ecommerce/app/(with_footer)/product/[slug]/page.tsx`
...
export default function ProductDetailScreen() {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      await addItem({ product: product._id, quantity: 1, price: product.price });
...
- Variant modal if product has variants
- Price with discount badge
- Rating stars
- Stock status indicator
- Add to cart button (opens variant modal if needed)
- Product fetched by ID or slug

## Verify

Product loads, images display, variants show modal, add to cart works.
