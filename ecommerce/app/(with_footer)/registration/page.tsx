...
const RegistrationPage = () => {
  ...
  const validate = (fields?: string[]): boolean => {
    ...
    const check = fields || ["name", "email"];

    if (check.includes("name") && !name.trim()) newErrors.name = "Name is required";
    if (check.includes("email")) {
      if (!email.trim()) newErrors.email = "Email is required";
      else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) newErrors.email = "Invalid email address";
    }
    if (check.includes("otp") && otp.length !== 6) newErrors.otp = "Enter the 6-digit OTP";
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!validate(["otp", "mobile", "password"])) return;

    setLoading(true);
    setErrors({});
    try {
      const res = await api.post("/auth/register-manually", {
        name,
        email,
        mobile,
        password,
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-2.5 border border-gray-300 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all disabled:bg-gray-50 disabled:text-gray-500"
                />
              </div>
            ...
            {!otpSent ? (
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 bg-blue-600 text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-blue-700 disabled:opacity-50 transition-colors"
              >
              ...
              <>
                <div className="px-2 py-3 rounded-lg bg-blue-50 border border-blue-200 text-blue-700 text-sm text-center">
                  A 6-digit code was sent to <span className="font-semibold">{email}</span>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Enter OTP</label>
                  <OTPInput value={otp} onChange={(val) => setOtp(val)} validTime={120} />
