...
export default class ProductService {
  static async create(payload: CreateProductInput) {
    ...
    await withTransaction(async (tx) => {
      ...
      await Promise.all(
        variants.map(async (v) => {
          ...
          const variantCreated = await ProductRepository.createVariant(
            variantPayload,
            tx,
          );
  ...
  static async update(
    id: number,
    payload: { productInput: UpdateProductInput; variants: VariantPayload[] },
  ) {
    const { productInput, variants } = payload;
    const hasProductFields =
      productInput && Object.keys(productInput).length > 0;
    ...
    await withTransaction(async (tx) => {
      if (hasProductFields) {
        ...
        if (productInput.name) {
          exist = await ProductRepository.findByFieldExceptId(
            "name",
            productInput.name,
            id,
          );
            ...
            newVariants.map(async (v) => {
              if (v.barcode) {
                ...
                if (exist) {
                  throw new ApiError(
                    400,
                    "Please try with some different barcode",
                  );
                }
              }
        ...
        if (oldVariantsToUpdate.length > 0) {
          await Promise.all(
            oldVariantsToUpdate.map(async (v) => {
              if (v.barcode) {
                const exist =
                  await ProductRepository.findVariantByBarcodeExceptID(
                    v.barcode,
                    v.id as number,
                  );
              ...
              const variantUpdate = await ProductRepository.updateVariant(
                id!,
                variantPayload,
                tx,
              );
  ...
  static async updateProductFifoBatchAndStock(
    ...
    return ProductRepository.updateProductFifoBatchAndStock(
      productID,
      options,
      tx,
    );
  ...
  static async getFifoBatchesByVariantID(variantID: number) {
    const batches = await ProductRepository.fifoBatchesByVariantID(variantID);
  ...
  static async findStockFlowDynamically(
    ...
    return ProductRepository.findStockFlowDynamically(
      batchID,
      column,
      columnID,
      tx,
